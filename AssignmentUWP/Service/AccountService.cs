...
namespace AssignmentUWP.Service
{
    public class AccountService
    {
            ...
            public async Task<bool> IsRegisterAsync(Account account)
            {
                ...
                var result = await httpClient.PostAsync($"{ Api.apiDomain }{Api.accountPathRegisterAndInfo}", contentToSend);
            ...
            public async Task<Credential> LoginAsync(string Email, string Password)
            {
                ...
                {
                    var result = await httpClient.PostAsync($"{ Api.apiDomain }{Api.accountPathLogin}", encodedContent);
            ...
            public async Task<Account> GetAccountInformation(string token)
            {
                using (HttpClient httpClient = new HttpClient())
                {
                    ...
                    var result = await httpClient.GetAsync($"{ Api.apiDomain }{Api.accountPathRegisterAndInfo}");
                    var content = await result.Content.ReadAsStringAsync();
                    if (result.StatusCode == System.Net.HttpStatusCode.OK)
                    {
                        // good case
